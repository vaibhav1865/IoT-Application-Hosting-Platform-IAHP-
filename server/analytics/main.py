...
def is_valid_developer(developer_id):
    try:
        ...
        logger.log(
            ...
            level=3,
            msg=f"Error checking developer: {e}"
        )
...
def is_valid_app(developer_id, app_id):
    try:
        ...
        logger.log(
            service_name=SERVICE_NAME,
            level=3,
            msg=f"Error checking app: {e}"
        )
        print(f"Error checking app:")
        return False

# print(is_valid_developer('6436bad3b8fe3565a2935744')) # true (developer exists)
# print(is_valid_app('6436bad3b8fe3565a2935744' , '644266929b608d3d4534cb67')) # true
# print(is_valid_app('643c60a198bf68ed7533b76a' , '644266929b608d3d4534cb67')) # false (developer exists but app does not belong to the develope
...
def get_apps(developer_id: str) -> List[str]:
    ...
    try:
        ...
        logger.log(
            service_name=SERVICE_NAME,
            level=3,
            msg=f"Error fetching apps: {str(e)}"
        )
...
@app.get("/analytics/waterSensor/pressureVoltage")
async def generate_performance_analysis_chart(developer_id: str, app_id: str, numSensors: int = 1, numTimeStamps: int = 5):
    if developer_id is None or app_id is None:
        ...
        return HTMLResponse(content="Error: Developer ID and App ID are required fields", status_code=400)
    ...
    if not is_valid_app(developer_id, app_id):
        ...
        return HTMLResponse(content="Error: App not found for this developer", status_code=404)

    try:
        # Call the function to get sensor data and labels
        sensor_ids, data_dict = get_sensor_data(
            "Flowrate", numSensors, numTimeStamps)
        ...
        return HTMLResponse(content=f"Error: Something missing/wrong: ", status_code=500)


@app.get("/analytics/waterSensor/heatmap")
async def generate_heatmap_chart(developer_id: str, app_id: str, fieldName="Total Flow", numSensors: int = 5, numTimeStamps: int = 10):
    if developer_id is None or app_id is None:
        logger.log(
            service_name=SERVICE_NAME,
            level=2,
                ...
                "heatmap": {
                    ...
                    "colorScale": {
                        "ranges": [
                            ...
                            {"from": 21, "to": 50, "name": "Medium",
                                "color": "#FFA500"},
                            {"from": 51, "to": 100, "name": "High",
                                "color": "#00FF00"},
                        ]
                    }
                }
...
@app.get("/analytics/smartRoom/energy")
async def generate_line_chart(developer_id, app_id, numSensors: int = 3, numTimeStamps: int = 15):
    if developer_id is None or app_id is None:
        logger.log(
            service_name=SERVICE_NAME,
            level=2,
    ...
    try:
        # Get data and labels from the data_dict based on developer_id and app_id
        sensor_ids, data_dict = get_sensor_data(
            "Energy", numSensors, numTimeStamps)
        datasets = []
...
@app.get("/analytics/smartRoom/power")
async def generate_doughnut_chart(developer_id, app_id, numSensors: int = 1, numTimeStamps: int = 5):
    numSensors = 1
    if developer_id is None or app_id is None:
        logger.log(
            service_name=SERVICE_NAME,
...
@app.get("/analytics/smartRoom/current")
async def generate_circle_radius_bar_chart(developer_id, app_id, numSensors: int = 1, numTimeStamps: int = 5):
    if developer_id is None or app_id is None:
        logger.log(
            service_name=SERVICE_NAME,
            level=2,
...
@app.get("/analytics/solarMonitor/heatmap")
